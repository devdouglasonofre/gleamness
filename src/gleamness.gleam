...
pub fn init(_flags: Nil) -> #(Model, effect.Effect(Msg)) {
  ...
  let cpu_with_game = case cpu.load(new_cpu, get_snake_game()) {
    Ok(loaded_game_cpu) -> loaded_game_cpu
    Error(_) -> new_cpu
  }

  let assert Ok(cpu_reset) = cpu.reset(cpu_with_game)
